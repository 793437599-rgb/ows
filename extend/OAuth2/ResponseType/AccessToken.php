...
class AccessToken implements AccessTokenInterface
{
    ...
    public function createAccessToken($client_id, $user_id, $scope = null, $includeRefreshToken = true)
    {
        ...
        if($user_id){
            $token['openid'] = $user_id;
        }

        $this->tokenStorage->setAccessToken($token["access_token"], $client_id, $user_id, $this->config['access_lifetime'] > 0?time() + $this->config['access_lifetime']:nul, $scope);
