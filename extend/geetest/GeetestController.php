...
//        $request->session()->put("userId", $userId);
    ...
    // 二次验证接口，POST请求
    public function second_validate()
    {
        $gtLib = new GeetestLib(GEETEST_ID, GEETEST_KEY);
        ...
        if ($status === null) {
            return json(["result" => "fail", "version" => GeetestLib::VERSION, "msg" => "session取key发生异常"]);
        }
        $result = null;
        if ($status === 1) {
            /*
            自定义参数,可选择添加
                ...
                ip_address 客户端请求sdk服务器的ip地址
            */
            $params = ["client_type" => "web", "ip_address" => "127.0.0.1"];
        ...
        if ($result->getStatus() === 1) {
            return json(["result" => "success", "version" => GeetestLib::VERSION]);
        } else {
            return json(["result" => "fail", "version" => GeetestLib::VERSION, "msg" => "请先完成滑块验证"]);
        }
    }
